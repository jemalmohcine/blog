...
<div class="container">
 <div class="row">
          <?php
          $id=$_GET['idd'];
          $sql = "SELECT idd,titre,description, image,categorie FROM blogpost WHERE idd=$id";
          ...
          <?php 
	       if (isset($_POST["addcoment"]))
 			{ 
				...
				$insert="INSERT INTO commentaire (commentaire,id_article) VALUES('$comment','$idAr') ";
				mysqli_query($conn,$insert);
				
	
			}
			$sql = "SELECT commentaire FROM commentaire WHERE id_article=$id ";
			$result = mysqli_query($conn, $sql);
			if (mysqli_num_rows($result) > 0) {
          while($row = mysqli_fetch_assoc($result)) {
          ?>  
